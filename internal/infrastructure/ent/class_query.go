...
func (_q *ClassQuery) QueryStudents() *StudentQuery {
...
func (_q *ClassQuery) QueryTeachers() *TeacherQuery {
...
func (_q *ClassQuery) QueryCourses() *CourseQuery {
...
func (_q *ClassQuery) Clone() *ClassQuery {
	...
	return &ClassQuery{
		...
		withTeachers: _q.withTeachers.Clone(),
...
func (_q *ClassQuery) WithStudents(opts ...func(*StudentQuery)) *ClassQuery {
	query := (&StudentClient{config: _q.config}).Query()
...
func (_q *ClassQuery) WithTeachers(opts ...func(*TeacherQuery)) *ClassQuery {
	query := (&TeacherClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTeachers = query
	return _q
}
...
func (_q *ClassQuery) WithCourses(opts ...func(*CourseQuery)) *ClassQuery {
	query := (&CourseClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ClassQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Class, error) {
	...
	if query := _q.withTeachers; query != nil {
		if err := _q.loadTeachers(ctx, query, nodes,
	...
	if query := _q.withCourses; query != nil {
		if err := _q.loadCourses(ctx, query, nodes,
			func(n *Class) { n.Edges.Courses = []*Course{} },
			func(n *Class, e *Course) { n.Edges.Courses = append(n.Edges.Courses, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ClassQuery) loadStudents(ctx context.Context, query *StudentQuery, nodes []*Class, init func(*Class), assign func(*Class, *Student)) error {
...
func (_q *ClassQuery) loadTeachers(ctx context.Context, query *TeacherQuery, nodes []*Class, init func(*Class), assign func(*Class, *Teacher)) error {
	...
	neighbors, err := withInterceptors[[]*Teacher](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
...
func (_q *ClassQuery) loadCourses(ctx context.Context, query *CourseQuery, nodes []*Class, init func(*Class), assign func(*Class, *Course)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Class)
	nids := make(map[int]map[*Class]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(class.CoursesTable)
		s.Join(joinT).On(s.C(course.FieldID), joinT.C(class.CoursesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(class.CoursesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(class.CoursesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
